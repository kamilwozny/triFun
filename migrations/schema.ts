...
export const eventAttendees = sqliteTable(
  ...
  (table) => {
    return {
      eventIdAttendeeIdUnique: uniqueIndex(
        'event_attendees_event_id_attendee_id_unique'
      ).on(table.eventId, table.attendeeId),
    };
  }
);
...
export const events = sqliteTable(
  'events',
  {
    ...
    isPrivate: integer('isPrivate').default(0).notNull(),
    price: integer('price'),
    status: text('status').default('draft').notNull(),
  },
  (table) => {
    return {
      createdByIdNameUnique: uniqueIndex('events_createdById_name_unique').on(
        table.createdById,
        table.name
      ),
    };
